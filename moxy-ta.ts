...
export class MoxyTA {
	...
	public scan(): IMoxyTAResult {
		...
		Object.keys(this._analysis.wordFrequency).forEach((k: string) => {
			this._analysis.totals.totalWords += this._analysis.wordFrequency[
				k
			].frequency
		})

		Object.keys(this._analysis.wordFrequency).forEach((k: string) => {
			this._analysis.topWords.push({
				word: k,
				frequency:
					this._analysis.wordFrequency[k].frequency /
					this._analysis.totals.totalWords,
			})
		})

		Object.keys(this._analysis.alphabetFrequency).forEach((k: string) => {
			this._analysis.totals.totalLetters += this._analysis.alphabetFrequency[
				k
			]
		})

		Object.keys(this._analysis.punctuationFrequency).forEach(
			(k: string) =>
				(this._analysis.totals.totalPunctuation += this._analysis.punctuationFrequency[
					k
				]),
		)

		Object.keys(this._analysis.alphabetFrequency).forEach((k: string) => {
			this._analysis.alphabetFrequencyPercent.push({
				letter: k,
				frequency:
					this._analysis.alphabetFrequency[k] /
